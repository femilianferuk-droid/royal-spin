...
def validate_init_data(init_data: str) -> dict | None:
    ...
    """
    if not BOT_TOKEN:
        # Если токен не задан (например, при локальной отладке без тг),
        # примем данные как есть — но это режим разработки.
        try:
      ...
      try {
        ...
        document.getElementById("loading").innerHTML =
          `<div style="color:#fca5a5; padding:20px; text-align:center">
            Не удалось авторизоваться.<br>Откройте приложение через Telegram-бота.<br>
...
def api_me():
    ...
    if not user_data or "user" not in user_data:
        return jsonify({"ok": False, "error": "Не удалось авторизоваться через Telegram"}), 401
...
def api_play():
    ...
    try:
        ...
        # В случае бага — возвращаем ставку
        new_balance = change_balance(user_id, bet)
