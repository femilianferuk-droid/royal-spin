...
# Таймаут подключения к PostgreSQL. Без него psycopg2 может зависнуть на десятки
# секунд (или минуты) при недоступном хосте, и фронт будет крутить "Загружаем..."
# бесконечно. Ставим 5 секунд — за это время успеваем и в живой БД, и быстро
# отваливаемся, если БД лежит.
...
def get_conn():
    ...
    connect_timeout важен: если БД недоступна (хост лежит, файрвол режет и т.п.),
    без таймаута psycopg2 будет висеть на TCP-хэндшейке минутами — и мини-апп
    будет бесконечно показывать "Загружаем...". С таймаутом за 5 сек получим
    нормальную ошибку, которую можно показать пользователю.
    """
...
<body>
  ...
  <script>
    ...
    // Заворачиваем fetch в AbortController с таймаутом. Без этого, если бэк
    // висит (например, БД недоступна и psycopg2.connect ждёт TCP-хэндшейк),
    // промис никогда не резолвится — и юзер бесконечно видит "Загружаем...".
    ...
    async function loadUser() {
      // Если открыли НЕ через Telegram (прямой URL в браузере, без бота) —
      // init_data пустой. Не делаем вид, что что-то загрузится, сразу
      ...
      try {
        ...
        if (!data.ok) {
          ...
          if (data.reason === "auth" && !INIT_DATA) {
            ...
            hint = "Возможно, бот мини-аппа и тот, из которого открываешь — разные.";
...
def health():
    """Эндпоинт для предварительной проверки (фронт стучится сюда перед /api/me).
    Возвращает JSON c ok/db_ok, чтобы клиент мог показать осмысленную ошибку,
...
# Одноразовая инициализация схемы БД. Раньше она стояла только в
# `if __name__ == "__main__"`, и при деплое через gunicorn (как реально и
# предполагается в requirements.txt) таблица users не создавалась — из-за чего
# первый же запрос валился с "relation does not exist". Теперь — сразу при
# импорте модуля, но в try/except: если БД пока недоступна, поднимемся и
# попробуем ещё раз на первом запросе (get_or_create_user/ping_db отработают).
try:
    ensure_schema()
